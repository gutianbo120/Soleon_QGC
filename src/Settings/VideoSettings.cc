...
#ifdef UDP_TRANSMIT
DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, transmitIP)
{
    if (!_transmitIPFact) {
        _transmitIPFact = _createSettingsFact(transmitIPName);
        connect(_transmitIPFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
    }
    return _transmitIPFact;
}

DECLARE_SETTINGSFACT_NO_FUNC(VideoSettings, transmitPort)
{
    if (!_transmitPortFact) {
        _transmitPortFact = _createSettingsFact(transmitPortName);
        connect(_transmitPortFact, &Fact::valueChanged, this, &VideoSettings::_configChanged);
    }
    return _transmitPortFact;
}
#endif
