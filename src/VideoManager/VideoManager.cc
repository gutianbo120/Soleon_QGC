...
#ifdef UDP_TRANSMIT
   connect(_videoSettings->transmitIP(),    &Fact::rawValueChanged, this, &VideoManager::_transmitIPChanged);
   connect(_videoSettings->transmitPort(),  &Fact::rawValueChanged, this, &VideoManager::_transmitPortChanged);
#endif
#if defined(QGC_GST_STREAMING)
   ...
   _updateUVC();
    ...
    connect(_videoReceiver[0], &VideoReceiver::onStartComplete, this, [this](VideoReceiver::STATUS status) {
        ...
        if (status == VideoReceiver::STATUS_OK) {
            ...
            if(hasVideo())
            {
                QString infoIP   = qgcApp()->toolbox()->settingsManager()->videoSettings()->transmitIP()->rawValue().toString();
                int     infoPort = qgcApp()->toolbox()->settingsManager()->videoSettings()->transmitPort()->rawValue().toInt();
                QString videoSource = _videoSettings->videoSource()->rawValue().toString();
                if(videoSource == VideoSettings::videoSourceUDPH265)
                {
                    _videoReceiver[0]->startUdpTransmitting(infoIP,infoPort,0);
                }
                else if(videoSource == VideoSettings::videoSourceUDPH264)
                {
                    _videoReceiver[0]->startUdpTransmitting(infoIP,infoPort,1);
                }
                else
                {
                    _videoReceiver[0]->startUdpTransmitting(infoIP,infoPort,0);
...
#ifdef UDP_TRANSMIT
void
VideoManager::startUdpTransmitting(const QString& ip,int port)
{
    if (qgcApp()->runningUnitTests()) {
        return;
    }
    ...
    if (_videoReceiver[0] && _videoStarted[0]) {
        if(hasVideo())
        {
            ...
            if(videoSource == VideoSettings::videoSourceUDPH265)
            {
                _videoReceiver[0]->startUdpTransmitting(ip,port,0);
            }
            else if(videoSource == VideoSettings::videoSourceUDPH264)
            {
                _videoReceiver[0]->startUdpTransmitting(ip,port,1);
            }
            else
            {
                _videoReceiver[0]->startUdpTransmitting(ip,port,1);
...
void
VideoManager::stopUdpTransmitting()
{
    if (qgcApp()->runningUnitTests()) {
        return;
    }
#if defined(QGC_GST_STREAMING)
    if (_videoReceiver[0]) {
        _videoReceiver[0]->stopUdpTransmitting();
    }
#endif
}
#endif
...
#ifdef UDP_TRANSMIT
void
VideoManager::_transmitIPChanged()
{
    _updateSettings(0);
    stopVideo();
}
...
#ifdef UDP_TRANSMIT
void
VideoManager::_transmitPortChanged()
{
