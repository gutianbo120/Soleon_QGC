...
GstVideoReceiver::GstVideoReceiver(QObject* parent)
    ...
    , _endOfStream(false)
#ifdef UDP_TRANSMIT
    , _udpTransmitting(false)
    , _removingUdpTransmit(false)
    , _rtpIsH264(false)
...
#ifdef UDP_TRANSMIT
    GstElement* udpSinkQuque = nullptr;
#endif
    do {
...
#ifdef UDP_TRANSMIT
        if((udpSinkQuque = gst_element_factory_make("queue", nullptr)) == nullptr)  {
            qCCritical(VideoReceiverLog) << "gst_element_factory_make('queue') failed";
            break;
        }

        if((_udpSinkValve = gst_element_factory_make("valve", nullptr)) == nullptr)  {
            qCCritical(VideoReceiverLog) << "gst_element_factory_make('valve') failed";
            break;
        }
...
#ifdef UDP_TRANSMIT
        gst_bin_add_many(GST_BIN(_pipeline), _source, _tee, decoderQueue, _decoderValve, recorderQueue, _recorderValve,udpSinkQuque,_udpSinkValve, nullptr);
#else
...
#ifdef UDP_TRANSMIT
        if(!gst_element_link_many(_tee, udpSinkQuque, _udpSinkValve, nullptr)) {
            qCCritical(VideoReceiverLog) << "Unable to link recorder queue";
            break;
        }
#endif
...
#ifdef UDP_TRANSMIT
            if (_udpSinkValve != nullptr) {
                gst_object_unref(_udpSinkValve);
                _udpSinkValve = nullptr;
            }

            if (udpSinkQuque != nullptr) {
                gst_object_unref(udpSinkQuque);
                udpSinkQuque = nullptr;
            }
#endif
...
#ifdef UDP_TRANSMIT
            gboolean udpSinkValveClosed = TRUE;
            g_object_get(_udpSinkValve, "drop", &udpSinkValveClosed, nullptr);

            if (udpSinkValveClosed == FALSE) {
                gst_element_send_event(_pipeline, gst_event_new_eos());
...
#ifdef UDP_TRANSMIT
void
GstVideoReceiver::startUdpTransmitting(const QString& ip,int port,int rtpType)
{
    qCDebug(VideoReceiverLog) << "Starting UdpTransmit" << _uri;
    switch(port)
    {
    case 0:
        g_object_set(_udpSinkValve, "drop", TRUE, nullptr);
    ...
    if (_pipeline == nullptr) {
        qCDebug(VideoReceiverLog) << "Streaming is not active!" << _uri;
        _dispatchSignal([this](){
            emit onStartUdpTransmittingComplete(STATUS_INVALID_STATE);
        });
        qDebug() << "startUdpTransmitting _pipeline is ull!!!";
        return;
    }

    if (_udpTransmitting) {
        qCDebug(VideoReceiverLog) << "Already _udpTransmitting!" << ip << ":" << port;
        qDebug() << "Already _udpTransmitting!!!!";
        _dispatchSignal([this](){
            emit onStartUdpTransmittingComplete(STATUS_INVALID_STATE);
        });
        qDebug() << "startUdpTransmitting Already _udpTransmitting!!!";
        return;
    }

    qCDebug(VideoReceiverLog) << "New UDP Transmit Ip:" << ip <<  ",port:" << port;

    if ((_udpSink = _makeUdpSink(ip, port)) == nullptr) {
        qCCritical(VideoReceiverLog) << "_makeUdpSink() failed" << _uri;
        _dispatchSignal([this](){
            emit onStartUdpTransmittingComplete(STATUS_FAIL);
        });
        qDebug() << "startUdpTransmitting _makeUdpSink failed";
        return;
    }
    _removingUdpTransmit = false;
    rtpType = _rtpIsH264?-1:0;
    _rtppay = gst_element_factory_make(rtpType==0?"rtph265pay":"rtph264pay",rtpType==0?"rtph265pay":"rtph264pay");
    if (!_rtppay) {
        qDebug() << "create rtph265pay error";
        return;
    }
    g_object_set(_rtppay,"config-interval", -1, NULL);
    g_object_set(_rtppay,"mtu",512,NULL);
    gst_object_ref(_rtppay);
    gst_object_ref(_udpSink);

    gst_bin_add(GST_BIN(_pipeline),_udpSink);
    gst_bin_add(GST_BIN(_pipeline),_rtppay);
    if (!gst_element_link(_udpSinkValve,_rtppay))
    {
        qCCritical(VideoReceiverLog) <<"Failed to link valve and udpsink 1--" <<ip<<":"<<port;
        _dispatchSignal([this](){
            emit onStartUdpTransmittingComplete(STATUS_FAIL);
        });
        //rtp type类型不匹配 重新配置类型
        ...
        if (!gst_element_link(_udpSinkValve,_rtppay))
        {
            qDebug() << "startUdpTransmitting Failed to link valve and _rtppay:2";
            return;
        }
        qDebug() << "startUdpTransmitting link valve and _rtppay:success!!!";
        //return;
    }
    ...
    {
        qCCritical(VideoReceiverLog) <<"Failed to link valve and udpsink" <<ip<<":"<<port;
        _dispatchSignal([this](){
            emit onStartUdpTransmittingComplete(STATUS_FAIL);
        });
        qDebug() << "startUdpTransmitting Failed to _rtppay and udpsink";
        return;
    }
    gst_element_sync_state_with_parent(_udpSink);
    GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(_pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "pipeline-with-udpsink");
    g_object_set(_udpSinkValve, "drop", FALSE, nullptr);
    _udpTransmitting = true;
    qCDebug(VideoReceiverLog) << "udp transmit started " <<ip<<":"<<port;
    qDebug()                  << "udp transmit started " <<ip<<":"<<port;
    _dispatchSignal([this](){
        emit onStartUdpTransmittingComplete(STATUS_OK);
        emit udpTransmittingChanged(_udpTransmitting);
    });
}
#endif
...
#ifdef UDP_TRANSMIT
void
GstVideoReceiver::stopUdpTransmitting(void)
{
    qCDebug(VideoReceiverLog) << "Stopping udpTransmitting";
    qDebug() << "Stopping udpTransmitting!!!!";
    //_shutdownUdpTransmitBranch();
    if (_pipeline == nullptr || !_udpTransmitting) {
        qCDebug(VideoReceiverLog) << "Not udpTransmitting!";
        qDebug() << "Not udpTransmitting!!!!";
        _dispatchSignal([this](){
            emit onStopUdpTransmittingComplete(STATUS_INVALID_STATE);
        });
        return;
    }
    g_object_set(_udpSinkValve, "drop", TRUE, nullptr);
    _removingUdpTransmit = true;
    _udpTransmitting     = false;
    bool ret = _unlinkBranch(_udpSinkValve);
    // FIXME: AV: it is much better to emit onStopRecordingComplete() after recording is really stopped
    // (which happens later due to async design) but as for now it is also not so bad...
    _dispatchSignal([this, ret](){
        emit onStopUdpTransmittingComplete(ret ? STATUS_OK : STATUS_FAIL);
    });
    qDebug() << "Stopping udpTransmitting end!!!!";
}
#endif
...
#ifdef UDP_TRANSMIT
GstElement* GstVideoReceiver::_makeUdpSink(const QString& ip,int port)
{
#if 0
    GstElement* udpSink = gst_element_factory_make ("fakesink", "fsink");
    if(!udpSink)
    {
        qCCritical(VideoReceiverLog) << "gst_element_factory_make('udpsink') failed";
        return udpSink;
    }
    g_object_set(udpSink,"state-error",4,NULL);
    return udpSink;
#else
    ...
    g_object_set(udpSink,"port",port,"host",ip.toUtf8().data(),NULL);
    g_object_set(udpSink, "async", FALSE, NULL);
    g_object_set(udpSink, "sync", 1, NULL);
    gst_object_ref(udpSink);
    return udpSink;
#endif
}
#endif
...
#ifdef UDP_TRANSMIT
void
GstVideoReceiver::_shutdownUdpTransmitBranch(void)
{
    gst_bin_remove(GST_BIN(_pipeline), _udpSink);
    gst_element_set_state(_udpSink, GST_STATE_NULL);
    gst_object_unref(_udpSink);
    _udpSink = nullptr;
    gst_bin_remove(GST_BIN(_pipeline), _rtppay);
    gst_object_unref(_rtppay);
    _rtppay= nullptr;

    _removingUdpTransmit = false;
    if (_udpTransmitting) {
        _udpTransmitting = false;
        qCDebug(VideoReceiverLog) << "_udpTransmiting stopped";
        qDebug() << "_udpTransmiting stopped..............";
        _dispatchSignal([this](){
            emit udpTransmittingChanged(_udpTransmitting);
        });
    }
    GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(_pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "pipeline-udpTransmitting-stopped");
    //设置采样回调
    //g_signal_connect(_udpSink, "new-sample", G_CALLBACK(ReadvideoFrame_callback),nullptr);
}
#endif
