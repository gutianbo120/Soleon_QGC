...
#ifdef UDP_TRANSMIT
    virtual void startUdpTransmitting(const QString& ip,int port,int rtpType);
    virtual void stopUdpTransmitting(void);
#endif
protected slots:
...
#ifdef UDP_TRANSMIT
    virtual GstElement* _makeUdpSink(const QString& ip,int port);
    virtual void _shutdownUdpTransmitBranch(void);
#endif
...
#ifdef  UDP_TRANSMIT
    bool                _udpTransmitting;
    bool                _removingUdpTransmit;
    bool                _rtpIsH264;
