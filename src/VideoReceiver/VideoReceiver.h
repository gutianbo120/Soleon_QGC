...
#ifdef UDP_TRANSMIT
    void udpTransmittingChanged(bool active);
    void onStartUdpTransmittingComplete(STATUS status);
    void onStopUdpTransmittingComplete(STATUS status);
#endif
public slots:
    // buffer:
...
#ifdef UDP_TRANSMIT
    virtual void startUdpTransmitting(const QString& ip,int port,int rtpType = 0) = 0;
    virtual void stopUdpTransmitting(void) = 0;
#endif
};
